...
public class GamePlay : GameRunning {
  ...
  public GamePlay(GameStateMachine m) : base(m) {
  ...
  public override IEnumerator run() {
    ...
    while (ParentMachine.Current.GetType() == GetType()) {
      ...
      if (ElapsedTime >= 3.0f && ParentMachine.Component.gameSpeed.value < 12.0f) {
        ParentMachine.Component.gameSpeed.value += 0.5f;
        ElapsedTime = 0;
      }
  ...
  public override void SpawnLevel() {
    ...
    LevelPiece p = new LevelPiece(ParentMachine.Component.levels[levno], lastPiece.getNextSpawnPosition());
    Addlevel(p);
  }
  void GameOver() {
