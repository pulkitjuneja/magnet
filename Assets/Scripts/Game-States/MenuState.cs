...
public class MenuState : GameRunning {
  ...
  public MenuState(GameStateMachine g) : base(g) {
    ...
    if (!PlayerPrefs.HasKey(GameStateManager.SCORE_KEY)) {
      isFirstTime = true;
      PlayerPrefs.SetInt(GameStateManager.SCORE_KEY, 0);
      PlayerPrefs.Save();
    }
  }
  ...
  public void navigateToPlay() {
    if (isFirstTime) {
      ...
      stateMachine.SetState(typeof(GamePlay), true, new object[] { GameStateMachine.instance });
  ...
  public override void SpawnLevel() {
    ...
    LevelPiece lp = new LevelPiece(ParentMachine.Component.levels[levno], lastPiece.getNextSpawnPosition());
    Addlevel(lp);
  }
